...
- 未在同一任务中更新 spec 和 package scripts 前，不要引入 React、Vue、CSS modules 或 bundler。Tailwind CSS 由 Kooboo 运行环境支持；不要在 layout 中添加 Tailwind CDN，也不要为了使用 utilities 额外添加 package dependency 或 build step，除非任务明确要求。
...
- 不要使用 Tailwind 任意断点变体，例如 `max-[960px]:flex-wrap` 或 `min-[720px]:grid`；Kooboo 当前 Tailwind 环境不支持这类 breakpoint variant，使用标准响应式前缀或 `site.css` media query。
...
- Layout 是否没有加载 Tailwind CDN，且仍然加载 `./site.css` 和 `./site.js`？
...
- 是否没有遗留 `max-[...]` 或 `min-[...]` Tailwind 任意断点变体？
